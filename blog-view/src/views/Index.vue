<template>
	<div class="site">
		...
		<!--APlayer-->
		<div class="m-mobile-hide">
			<meting-js :server="siteInfo.playlistServer" :id="siteInfo.playlistId" type="playlist" fixed="true"
				theme="#25CCF7" v-if="siteInfo.playlistServer && siteInfo.playlistId"></meting-js>
		</div>
		<!--回到顶部-->
...
import { defaultSiteInfo, defaultIntroduction } from '@/settings'
		...
		getSite() {
			...
			getSite().then(res => {
				if (res.code === 200) {
					...
					this.$store.commit(SAVE_INTRODUCTION, defaultIntroduction)
